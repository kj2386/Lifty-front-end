        ...
        <>
            ...
            {data.length > 0 && (
                <table className="tbl-workout-log" border="2" bordercolor="#000">
                    ...
                    <tbody>
                        {data.map(({ _id, name, sets } = {}, workoutIndex) => (
                            <Fragment key={'exercise_' + _id}>
                                ...
                                {sets && sets.length > 0 && sets.map(({ _id, setNumber, reps, weight }, index) => (
                                    <tr key={'set_' + _id}>
                                        ...
                                        <td align="center">
                                            <a href="#" className="btn-delete" onClick={() => onDelete(workoutIndex, index)}>Delete</a>
                                            <a href="#"  onClick={() => onUpdate(workoutIndex, index)}>Update</a>
                                        </td> 
                                    </tr>
                                ))}
                            </Fragment>
